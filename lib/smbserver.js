...
function SMBServer(config, authenticator) {
  ...
  this.AddShare = function(share) {
    var name = share.name.toUpperCase();
    var smbShare = new SMBShare(self, share);
    self.shares[name] = smbShare;
    var shareEvents = smbShare.getEvents();

    var forwardEvent = function (eventName) {
      smbShare.on(eventName, function (arg) {
  ...
  function addShare(shareCfg, name) {
    var type = shareCfg.backend;
    var Share = require('./backends/' + type + '/share');
    var share = new Share(name, shareCfg);
    self.AddShare(name, share);
  };

  _.forEach(config.shares, addShare);
